...
MODULE_DESCRIPTION("MMAP module");
...
#else
static asmlinkage unsigned long (*real_sys_mmap)(unsigned long addr, unsigned long len,
		int prot, int flags,
		int fd, long offset);

static asmlinkage unsigned long fh_sys_mmap(unsigned long addr, unsigned long len,
		int prot, int flags,
		int fd, long offset)
{
	long ret;
	...
	ret = real_sys_mmap(addr, len, prot, flags, fd, offset);
	if (ret < 0) {
		error(abs(ret));
		return ret;
    }
