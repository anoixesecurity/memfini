...
MODULE_DESCRIPTION("MMAP2 module");
...
static asmlinkage long (*real_sys_mmap_pgoff)(struct pt_regs *regs);

static asmlinkage long fh_sys_mmap_pgoff(struct pt_regs *regs)
{
	long ret;
	ret = real_sys_mmap_pgoff(regs);
	if (ret < 0) {
		error1(abs(ret));
		return ret;
    }
	if (strncmp(current->comm, "demo1", 5) == 0) {
		...
		pr_info(",mem_alloc,%s(%d),0x%lx,%ld,permission%ld,%lx,%ld\n",current->comm, current->pid, ret, regs->si, regs->dx, regs->cx, regs->r8);

	}
	return ret;
}
#else
static asmlinkage unsigned long (*real_sys_mmap_pgoff)(unsigned long addr, unsigned long len,
		int prot, int flags,
		int fd, long offset);

static asmlinkage unsigned long fh_sys_mmap_pgoff(unsigned long addr, unsigned long len,
		int prot, int flags,
		int fd, long offset)
{
	long ret;
	...
	ret = real_sys_mmap_pgoff(addr, len, prot, flags, fd, offset);
	if (ret < 0) {
		error1(abs(ret));
		return ret;
    }
	pr_info(",mem_alloc,%s(%d),0x%lx,%ld,permission%ld,%lx,%ld\n",current->comm, current->pid, ret, len, prot, flags, fdk );
	return ret;
}
#endif
